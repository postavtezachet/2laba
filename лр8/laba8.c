...
int main() {
	...
	do {// Ввод цвета
		...
		scanf("%d", &a);// Ввод цвета фона выделенного слова
		printf("Выберите свой второй любимый цвет (цвет текста): ");
		scanf("%d", &b);// Ввод цвета текста выделенного слова
	} while (((a < 0) || (a>14)) || ( (b < 0) || (b>14))); // если введено число не от 0 до 14 то заново запрашивает цвет
	system("CLS"); //очистка экрана консоли
	char** str; //Указатель на указатель
	int n,r=0,m,l=0;
	printf("Введите количество строк ");
	scanf("%d", &n);
	str = (char**)malloc(n* sizeof(char*)); // выделяем память под количество строк
	printf("Введите размер строки ");
	scanf("%d", &m);
	for (int i = 0; i < n; i++) {
		str[i] = (char*)malloc(m+1 * sizeof(char)); // выделяем память под строки
	}
